...
MCP `get_feed_detail` 的笔记、`commentList` 中的评论及各层 `replies` 均通过 `time`
返回发布时间，格式为 `YYYY-MM-DD HH:mm`（北京时间，UTC+8）；日期缺失时省略该字段。
...
`get_feed_image` 返回一张实际的 MCP 图片内容，供支持视觉的客户端/模型查看：

```json
{"ref":"搜索或详情返回的笔记ref","image_index":1}
```

也可传 `feed_id` 和 `xsec_token`。序号从 1 开始，默认第 1 张，每次只取一张。
仅在明确需要识图、读图中文字/OCR、分析配色构图或比较图片，且文字详情不足时调用；
普通搜索、正文总结和互动不应自动取图，也不应遍历全部图片。
每次调用会重新访问笔记详情并下载图片，增加流量、延迟和视觉 token/上下文消耗；
具体费用依模型和客户端而定。图片下载超时 30 秒，单图上限 10 MiB。
`get_feed_detail` 默认仍只返回图片张数，`include_images=true` 只提供地址和尺寸。
本工具不提取视频帧或 Live Photo 动态内容。
